...
# U_z is EXCLUDED, on the data. Raw FlowTracker VelZ on this campaign averages
# |0.255| m/s with values to -0.598 m/s in a 0.5-0.7 m deep reach - that is not
# vertical velocity, it is probe tilt/noise. The model gives 0.001 m/s, which is
# the physically right answer, so the two can never agree and a third of the
# observations would be pure noise in the joint likelihood. Still extracted, so
# the profiles figure can show it.
CALIBRATION_QUANTITIES = ["U_x", "U_y"]
...
# Sized from ONE timed run at exactly the settings above, not from another reach:
# 70,262 cells on 8 cores took 407 s wall-clock for 600 s of simulated time
# (1,481 adaptive steps, dt ~0.42 s). So 48 runs is ~5.4 h.
# END_TIME is justified by the same run: mean |U| was flat to within 0.05% over the
# last 100 s, while the per-write change held at ~1.2% and did NOT decay - i.e. the
# flow is steady in the mean with persistent local fluctuation, which is exactly
# what N_AVG_TIMESTEPS=5 (the last 100 s) averages out. Note the discharge balance
# is NOT evidence here: a rigid lid fixes the water volume, so inflow equals
# outflow by construction (report.DischargeHistory says so).
INIT_RUNS: int | None = 32
MAX_RUNS: int | None = 48
