...
from axqua.logsetup import reset_logging
...
def _isolate_axqua_dirs(tmp_path_factory, monkeypatch):
    ...
    for var, name in (("AXQUA_CONFIG_DIR", "config"),
                      ("AXQUA_DATA_DIR", "data"),
                      ("AXQUA_STATE_DIR", "state"),
                      ("AXQUA_CACHE_DIR", "cache"),
                      ("AXQUA_JOB_ROOT", "jobs")):
...
def fake_case(tmp_path, fake_pysource):
    ...
    # `axqua <config> --check` validates that they exist, and that check is part of
...
def has_systemd_user() -> bool:
    ...
    from axqua.jobs.launchers.systemd import SystemdUserLauncher
