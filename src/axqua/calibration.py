...
and BAL settings declared in the axqua config.
"""
...
def emit_hbc_config(cfg: Config, calibration_csv: Path | None) -> Path:
    ...
    text = f'''"""HydroBayesCal TELEMAC config generated by axqua for case '{cfg.name}'.
...
# Emitting it here rather than relying on axqua's source patch of the staged
...
# n_last is unused while output_extraction_time is 'last'; 1 is the value that keeps
# the intent if anyone flips this back to 'mean_last'.
extraction = {{
