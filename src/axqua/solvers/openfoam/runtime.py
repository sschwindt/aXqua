...
Mirrors :class:`axqua.env.TelemacRuntime` and shares its
:class:`~axqua.env.ShellRuntime` base: axqua never imports OpenFOAM's
environment, it sources ``etc/bashrc`` in a subshell per command. That keeps
OpenFOAM's ``$PATH``, ``$LD_LIBRARY_PATH`` and MPI wrappers out of axqua's own
...
class OpenFoamProgress:
    ...
    Shares :class:`axqua.progress.ProgressBar` with the TELEMAC renderer, so a
    ...
    def __init__(self, end_time: float, *, start_time: float = 0.0, out=None,
        ...
        from axqua.progress import ProgressBar
...
class OpenFoamRuntime(ShellRuntime):
    ...
    def run_stage(self, case_dir: str | Path, stage: str, *, end_time: float,
        ...
        :func:`axqua.solvers.openfoam.dicts.activate`).
