...
failed run. ``checkMesh`` is still run by :mod:`axqua.solvers.openfoam.runtime` and
remains the authority; this is the fast pre-check, exactly as
:mod:`axqua.mesh_quality` is for the TELEMAC mesh.
...
def assess(of_mesh, *, state=None, roughness_constant: float = 0.5) -> MeshReport:
    """Measure *of_mesh* (an :class:`~axqua.solvers.openfoam.mesh.OpenFoamMesh`).
...
def _assess_wall_function(of_mesh, report: MeshReport, *, state,
    ...
    from axqua.solvers.openfoam.mesh import column_corner_vertices
