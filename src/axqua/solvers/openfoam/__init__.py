...
* **The mesh is extruded, not snapped.** A river bed is a single-valued height field,
  ...
  :mod:`~axqua.solvers.openfoam.polymesh` for why that avoids snappyHexMesh's
...
* **The wall function constrains the mesh, not the other way round.** On a gravel bed
  ...
  :mod:`~axqua.solvers.openfoam.quality` reports where that bites.

Attribute access is **lazy** (PEP 562). That is not decoration: this package's
sibling :mod:`~axqua.solvers.openfoam.spec` is read by ``axqua status`` and,
...
def __getattr__(name: str):
    ...
    if module is not None:
        return getattr(importlib.import_module(
            f"axqua.solvers.openfoam.{module}"), name)
    if name in _SUBMODULES:
        return importlib.import_module(f"axqua.solvers.openfoam.{name}")
    raise AttributeError(
        f"module 'axqua.solvers.openfoam' has no attribute {name!r}")
...
if TYPE_CHECKING:  # give type checkers and IDEs the real names
    from axqua.solvers.openfoam.case import (
        build_case, estimate_cells, OpenFoamArtifacts, summarise,
    )
    from axqua.solvers.openfoam.hotstart import load_hotstart, State2D
