...
from axqua.solvers.telemac.boundary import LiquidBoundary
...
def _channel_cell_size(cfg: Config, mesh) -> float:
    ...
    (:func:`axqua.mesh.nominal_channel_size` - gpkg per-zone sizes and
    ``mesh.size_scale`` included, so a scaled convergence level is seen)."""
    import numpy as np

    from axqua.solvers.telemac import mesh as mesh_mod
...
def _friction_rows(cfg: Config) -> list[tuple[int, int, float, str]]:
    ...
    if cfg.geodata.roughness_zones is not None and cfg.geodata.roughness_table is not None:
        from axqua.solvers.telemac.mesh import read_roughness_table
...
def _liquid_node_mask(cfg: Config, mesh, kind: str, tol: float | None = None):
    ...
    from axqua.solvers.telemac import boundary as bnd
...
def _node_roughness(cfg: Config, mesh):
    ...
    node falls in (set by :func:`axqua.mesh.interpolate_roughness`). Without
    roughness zones the lateral-boundary roughness is converted to an equivalent ks,
    the same fallback :func:`axqua.rating.synthesize_outflow_rating_from_section`
    uses.
    """
    import numpy as np

    from axqua.rating import _resolve_n
...
def _prewet_discharge(cfg: Config) -> float | None:
    ...
    """
    from axqua.workflow import resolve_discharge
...
def _normal_depth_prewet_depth(cfg: Config, mesh, mask, discharge):
    ...
    case *discharge* follows from :func:`axqua.rating.stage_for_discharge` (the
...
def write_liquid_boundaries(cfg: Config, liquids: list[LiquidBoundary],
    ...
    lines = ["# liquid-boundaries hydrograph generated by axqua", " ".join(header),
             " ".join(units)]
...
def write_source_regions(cfg: Config, regions: list) -> Path:
    ...
    **No blank lines, and no line may start with a space or tab.**
    ...
    """
    lines = ["# axqua: internal source/sink regions (losing-gaining reach)"]
...
def write_cas(cfg: Config, liquids: list[LiquidBoundary],
    ...
    :mod:`axqua.solvers.telemac.unsteady` writes the recommended one hotstarted from the steady
    result.
    ...
    *sections_input* / *sections_output* wire the CONTROL SECTIONS keywords so the run
    reports the flux across each open boundary (see :func:`axqua.unsteady`).
    *source_regions* (a :class:`axqua.boundary.InternalSourceRegion` list) adds
    ...
    routine (see :mod:`axqua.solvers.telemac.fortran`).
    """
    h = cfg.hydrodynamics
    ...
    if previous_computation:
        ...
        initial_conditions = [
            hotstart_note or "/ pre-wetted hotstart (channel seeded with water; see axqua)",
    ...
    lines: list[str] = [
        "/" + "-" * 68,
        f"/ TELEMAC2D steering file generated by axqua for case '{cfg.name}'",
        f"/ regime: {regime}",
        "/" + "-" * 68,
        ...
        *([f"LIQUID BOUNDARIES FILE : {liquid_boundaries_file}"]
        ...
        # Q_in(t) vs Q_out(t) for the unsteady run) - see axqua.unsteady.
        ...
        *(["VARIABLE TIME-STEP : YES",
        ...
        # (axqua.flux_convergence). Only emit it for a steady, FIXED-step run.
    ...
    if (cfg.gain_lose.active and cfg.gain_lose.implementation == "fortran"
            and source_regions):
        # percolation via a generated USER_RAIN routine (see axqua.fortran):
...
def write_gaia_cas(cfg: Config) -> Path | None:
    ...
    * ``PRESCRIBED SOLID DISCHARGES`` imposes a sediment supply per liquid boundary
    ...
    (``unsteady2d.cas`` / ``unsteady3d.cas``) - see :class:`axqua.config.Morphodynamics`.
    ...
    lines = [
        "/ GAIA steering file generated by axqua",
