...
* writes the geometry ``.slf`` via :mod:`axqua.core.selafin`.
...
from axqua.logsetup import log_step
...
# These helpers moved to axqua.core.geodata when the fourteen scattered layer
# readers were unified behind one cached Dataset. They stay reachable from here
# because existing code and tests import axqua.mesh._parse_decimal and friends.
...
def build_mesh(cfg: Config, dem_initial_roi: Path | None = None) -> Mesh:
    ...
    from axqua.solvers.telemac import mesh_quality
...
def interpolate_elevations(mesh: Mesh, dem_path: Path, *, decimals: int = 4,
    ...
    With *cfg*, the case's structures (:mod:`axqua.core.structures`) are burnt
...
# sample_raster_at moved to axqua.core.raster (both meshers need it and
