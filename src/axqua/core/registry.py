...
ENTRY_POINT_GROUP = "axqua.solvers"
...
_BUILTIN_SPECS: tuple[str, ...] = (
    "axqua.solvers.telemac.spec:SPEC",
    "axqua.solvers.openfoam.spec:SPEC",
)
...
class BackendSpec:
    ...
    def check_environment(self, cfg) -> bool | None:
        ...
        Probes the configured environment (:mod:`axqua.core.environment`) and
        ...
        for it (``axqua status --check-env``), because it spawns a shell.
        """
        try:
...
class SolverBackend(Protocol):
    """What an implementation must provide to be driven by axqua.
    ...
    comes from :mod:`axqua.core`, so a new backend implements the parts that are
...
def _load_spec(target: str) -> BackendSpec | None:
    ...
    try:
        ...
        log.warning("could not load solver spec %s: %s", target, exc)  # break axqua
        return None
