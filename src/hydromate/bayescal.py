...
def hbc_env() -> str:
    ...
    HydroBayesCal is meant to be pip-installed *into* hydromate's own environment;
    where that has been done, launching it in a separate env would run a different
    (possibly absent) install. So when ``hydroBayesCal`` imports here, default to
    the env this interpreter is in, and only fall back to the historical separate
    env when it does not.
    """
    if (env := os.environ.get("HYDROBAYESCAL_ENV")):
        return env
    import importlib.util
    import sys
    if importlib.util.find_spec("hydroBayesCal") is not None:
        current = os.environ.get("CONDA_DEFAULT_ENV") or Path(sys.prefix).name
        if current:
            return current
    return "wrr-proj"
