"""Tabs generated from axqua's capability matrix, not from a hardcoded list.

``axqua case-status --json`` reports, per solver, every capability on three axes -
...
* a capability that is ``no`` - not implemented *yet* - is **shown, disabled, with the
  ...
  rather than two, and it is the difference between "axqua cannot do this" and
  "axqua cannot do this *here*";
* adding a capability or a whole solver to axqua surfaces in the plugin **with no
...
#: its own name), because the whole point is that axqua may grow one we have never
#: heard of.
TITLES = {
    "steady2d": "Steady 2D",
...
class SolverView:
    ...
    def visible_capabilities(self) -> list[CapabilityView]:
        ...
        # Anything axqua knows about that this plugin does not goes on the end,
